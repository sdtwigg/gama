...
object Vec {
  ...
  implicit def regenerater[D<:Data: Regenerate]: Regenerate[Vec[D]] = new Regenerate[Vec[D]] {
    def regenerate(in: Vec[D], xform: NodeSpell[_<:Node])(implicit em: EnclosingModule): Vec[D] =
      new Vec(in.elements.map(implicitly[Regenerate[D]].regenerate(_,xform)))(Vectorizable.vectorizer[D](in.eltmuxer),em)
  }
